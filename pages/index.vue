<template>
  <h1 class="font-rajdhani capitalize">Dashboard</h1>
  <p class="my-3 font-bold">Buttons In WebSite</p>
  <div class="flex gap-1 justify-start items-center">
    <UButton color="orange" size="lg" variant="outline">Button</UButton>
    <UButton color="blue" size="lg">Button</UButton>
    <UButton color="green" size="md"
      >+ Define Booking Limit time for player type</UButton
    >
    <UButton color="red" size="sm">Button</UButton>
    <UButton color="orange" size="xs">Button</UButton>
    <UButton>Button</UButton>
  </div>

  <hr class="my-10" />

  <p class="my-3 font-bold">Inputs In WebSite</p>
  <div class="flex gap-1 justify-start flex-col">
    <UInput type="email" icon="icon-jigsaw-email" color="gray" size="xs" />
    <UInput type="email" icon="icon-jigsaw-email" color="gray" size="sm">
    </UInput>
    <UInput type="email" icon="icon-jigsaw-email" color="gray" size="md">
      <template #trailing>
        <span
          class="text-[#2C3B84] py-2 px-4 bg-[#eeeff5] rounded-[10px] font-bold leading-[16px] text-sm"
          >HRs</span
        >
      </template>
    </UInput>
    <UInput type="email" icon="icon-jigsaw-email" color="gray" size="lg">
      <template #trailing>
        <span
          class="text-[#2C3B84] py-2 px-4 bg-[#eeeff5] rounded-[10px] font-bold leading-[16px] text-sm"
          >Days</span
  ...
  <UFormGroup name="password" label="Your password" class="mb-8">
    <CustomInput icon="icon-jigsaw-password" color="gray" type="password" />
  </UFormGroup>

  <UFormGroup name="select" label="select something" class="mb-8">
    <CustomSelect
      icon="icon-jigsaw-flag"
      :options="[
        'Wade Cooper',
        'Arlene Mccoy',
        'Devon Webb',
  ...
  <DashboardProfileCard>
    <template #infoProfile>
      <div class="flex gap-6.5 items-center">
        <div class="w-22 h-22 rounded-full">
          <UAvatar
            alt="Nuha Orabi "
            :ui="{ text: 'text-red-100', placeholder: 'text-[#E1FF29]' }"
            class="w-full h-full text-4xl bg-[#3E6A36] text-[#E1FF29] text-[22px]"
          />
        </div>
        <div class="flex gap-4 flex-col">
          <h3 class="text-[#3E6A36] font-bold text-[24px] leading-6">
            Nuha Orabi
          </h3>
          <UButton color="green" size="xs">Active</UButton>
        </div>
      </div>
    </template>
    <template #input>
      <UButton color="blue" size="sm" class="relative w-fit cursor-pointer"
        >Change Picture
        <input
          type="file"
          name=""
          id=""
          class="absolute top-0 w-full h-full left-0 opacity-0 cursor-pointer"
        />
      </UButton>
    </template>
  </DashboardProfileCard>
  ...
  <DashboardHeaderPage>
    <template #title>
      <div>
        <h2 class="text-[#3E6A36] font-bold text-[18px]">
          General Information
        </h2>
      </div>
    </template>
    <template #input>
      <div>
        <UButton color="blue" size="sm">Update</UButton>
      </div>
    </template>
  </DashboardHeaderPage>
  ...
  <UAccordion :items="items" class="accordion">
    <template #default="{ item, open }">
      <UButton
        color="gray"
        variant="ghost"
        class="py-7 px-7.5 rounded-full flex items-center justify-between bg-[#F6F6F6] mb-2 w-full"
        :ui="{ rounded: 'rounded-none', padding: { sm: 'p-3' } }"
      >
        <span class="text-[#3E6A36] font-bold text-[18px] block">{{
          item.label
        }}</span>
        <template #trailing>
          <div class="p-1 bg-[#3E6A36] rounded-full">
            <UIcon
              name="i-heroicons-chevron-right-20-solid"
              class="w-6 h-6 ms-auto transform transition-transform duration-200 bg-[#E1FF29]"
  ...
  <UToggle v-model="selected" size="2xl" />
</template>
