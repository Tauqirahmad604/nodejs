<script setup>
definePageMeta({
  title: 'players memberships',
  id: 'players-memberships',
  parent: { label: 'general settings', id: 'general-settings' },
  breadcrumb: [
    { label: 'home', to: '/' },
    { label: 'players memberships' },
  ]
})
...
const pageTotal = ref(players?.length)
const pageFrom = computed(() => (page.value - 1) * pageCount.value + 1)
const pageTo = computed(() => Math.min(page.value * pageCount.value, pageTotal.value))

const selected = ref([players[1]])
  ...
  <div>
    <div class="flex flex-col gap-y-5">
      <div class="flex items-center">
        ...
        <div class="flex-shrink-0">
          <UButton label="Add New Player" icon="icon-jigsaw-plus" color="cbgreen" size="sm" :ui="{ padding: { sm: 'ps-4 pe-5  py-2.5' } }" />
        </div>
      </div>
      <UTable v-model="selected" :columns="columns" :rows="players">
        <template #name-data="{ row }">
          <div class="flex items-center">
            ...
            <div class="flex-1">
              ...
              <span class="text-gray-400 block">ID: {{ row?.display_id }}</span>
            </div>
          </div>
        </template>
        <template #membership-data="{ row }">
          <div class="flex flex-col">
            <p class="capitalize">{{ row?.type ? row?.type : '---' }}</p>
            <span class="text-gray-400 block capitalize">{{ row?.membership }}</span>
          </div>
        </template>
        <template #phone-data="{ row }">
          <span class="block">{{ row?.phone ? row?.phone : '---' }}</span>
        </template>
        <template #gender-data="{ row }">
          <span class="block capitalize">{{ row?.gender }}</span>
        </template>
